...
<h1>Criar Pergunta Discursiva</h1>
    <form action="" method="POST">
        ...
        <label for="resposta">Opcao A:</label>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($id) && !empty($pergunta) && !empty($resposta) && !empty($opcaoA) && !empty($opcaoB) && !empty($opcaoC) && !empty($opcaoD)) {
            $linha = "ID: $id | Pergunta: $pergunta | Resposta: $resposta | Opção A: $opcaoA | Opção B: $opcaoB | Opção C: $opcaoC | Opção D: $opcaoD\n";
            $arquivo = fopen("Pergunta_MS.txt", "a"); 
